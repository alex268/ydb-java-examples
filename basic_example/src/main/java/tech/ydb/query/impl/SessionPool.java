...
class SessionPool implements AutoCloseable {
    ...
    private class PooledQuerySession extends SessionImpl {
        ...
        private final CompletableFuture<Void> waitFinish = new CompletableFuture<>();
        ...
        private void clean() {
            logger.info("QuerySession[{}] attach stream is stopped", getId());
            waitFinish.complete(null);
            isStopped = true;
            if (!isStarted) {
                destroy();
            }
        ...
        public void destroy() {
            ...
            logger.info("QuerySession[{}] STATS {} {}", getId(), deleted.get(), closed - started);
        }
